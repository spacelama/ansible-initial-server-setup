...
#         trap 'calling received signal
#               precmd_hook
#               called handled signal' SIGRTMIN
# # oh is that not cute?  `jobs` output and child numbering %3, %6, %9 etc is because we have our temporary signal handling children operating at the time that the child is forked.  We can at least fix `jobs` output with this stupid hack, but we cant fix child numbering:
# #              HISTTIMEFORMAT= history 1
# #              if ! HISTTIMEFORMAT= history 1 | grep -q "^ *[0-9]* *jobs" ; then
# #              kill -SIGRTMIN  $$
# #              fi
#         PS0='$(
#                calling sending signal
#                kill -SIGRTMIN  $$
#                called back from sending signal
#             )'
...
function precmd_hook() {
    ...
    calling precmd_hook/choosehistoryfile
    choosehistoryfile
    called precmd_hook/choosehistoryfile
    calling precmd_hook/writetohistory
    writetohistory
    called precmd_hook/writetohistory
...
#    case "${BASH_COMMAND%% *}" in  # FIXME: only relevant in trap DEBUG case I think
