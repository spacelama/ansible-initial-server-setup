...
function setup_display() {
    if [ -n "$DISPLAY" ] ; then
        ...
        if [ -z "$XAUTHORITY" ] ; then
            # assume the system has already set us up a default
            # working configuration, but we will soon override it:
        ...
        if [ "$XAUTHORITY" != "/tmp/.$USER.$DISPLAY" ] ; then
            ...
            XAUTHORITY=/tmp/.Xauthority.$USER.$DISPLAY ; export XAUTHORITY
            touch $XAUTHORITY
            if [ -n "$auth" ] ; then
        ...
        if [ "$PUSER" != root -a "$PUSER" != "" -a "$PUSER" != "$USER" ] ; then
            #  /tmp/.Xauthority.$PUSER.tmp is generated by sillysu()
            # export XAUTHORITY=`eval echo ~"$USER"/.Xauthority`
            #FIXME: times out
            ( cat /tmp/.Xauthority.$PUSER.tmp | timeout -k 1 1 xauth merge - ) >& /dev/null
            # just cleaning up something that's now unneeded since
            # we've dealt with it, to save confusion.  Sure there's a
            # race condition here, but sillysu is a manual thing:
            rm -f /tmp/.Xauthority.$PUSER.tmp
        fi
    fi
