 ...
 * Generalised (STDOUT,STDERR, portability, documentation) 2004-2006,2025
...
/* ANSI escape codes for our colour highlighting */
...
main (int argc, char **argv)
{
    ...
    if (f)
    {
        // If we're the parent, wait until there's stdout and/or
        // stderr available for us to read, read and write stderr
        // first, then write stdout so they're not intermingled,
        // ensuring open and close ANSI colour codes are properly
        // sequenced
        ...
        // keep reading our command's output in chunks as they become
        // available in turn, bracketing each chunk with the
        // appropriate STDOUT/STDERR codes.  Exit condition is if both
        // read streams are EOF, or one read stream errors (bug?
        // Should it exit only once both streams have errored?).
        do
        {
            tv.tv_sec=0;
            tv.tv_usec=10000;
    ...
    {
        // If we're the child, attach our output the the two
        // previously created pipes, and exec our command
